...
public class BinasClientAuthorizationHandler implements SOAPHandler<SOAPMessageContext> {
	...
	private boolean handleOutboundMessage(SOAPMessageContext smc) {
		...
		try {
			...
			String ticketString = new CipherClerk().cipherToString(cipheredTicket);
			...
			String authString = new CipherClerk().cipherToString(cipheredAuth);
